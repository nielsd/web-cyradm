...
if ($authorized) {
	...
	$query = "SELECT * FROM domain AS a WHERE domain_name LIKE '%".addslashes((string) $_GET['searchstring'])."%' AND ".$allowed_domains1."') ORDER BY domain_name";
	...
	$query = "SELECT DISTINCT a.username, a.domain_name FROM `virtual` as v, accountuser as a WHERE ((v.username LIKE '%".addslashes((string) $_GET['searchstring'])."%') OR (v.alias LIKE '%".addslashes((string) $_GET['searchstring'])."%')) AND (v.username=a.username) AND ".$allowed_domains1."') ORDER BY username";
	$result = $handle->query($query);
	...
	if (empty($row_pos)) {
        ...
        $query = "SELECT DISTINCT a.* FROM `virtual` as v, accountuser as a WHERE ((v.username LIKE '%".$_GET['searchstring']."%') OR (v.alias LIKE '%".addslashes((string) $_GET['searchstring'])."%')) AND (v.username=a.username) AND ".$allowed_domains1."') ORDER BY username";
	...
	if ($cnt!=0) {
		...
		for ($c=0;$c<$cnt;$c++) {
			...
			if (is_array($row)) {
				...
				$forwards_tmp = preg_explode('|,\s*|', stripslashes((string) $row['dest']));
				$forwards = [];
				foreach ($forwards_tmp as $forward) {
        if (strtolower((string) $forward) != strtolower((string) $username)){
   						$forwards[] = htmlspecialchars(trim((string) $forward));
   					} else {
   						$forwards[] = "<b>".htmlspecialchars(trim((string) $forward))."</b>";
   					}
    }
