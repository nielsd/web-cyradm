		...
		if (empty($_POST['confirmed'])){
			...
			if ($cnt+1 > $maxaccounts){
				...
				if (!$DOMAIN_AS_PREFIX){
					...
					$test = preg_match ('#[0-9][0-9][0-9][0-9]$#m', (string) $lastaccount, $result_array) ? strlen((string) $result_array[0]) : false;
				...
				<form action="index.php" method="POST" style="border: ridge 0px maroon;">
					...
					<table>
						...
						$_fields = ['email'	=> [_("Email address"), 'a', false, '@' . $_GET['domain']], 'quota' => [_("Quota"), '8', false, '', $def_quota], 'password' => [_("Password"), 'c', true, ''], 'confirm_password' => [_("Confirm Password"), 'c', true, '']];
			...
			if (!empty($folders)) $AUTOCREATE_MAILBOXES = explode(',',(string) $folders);
