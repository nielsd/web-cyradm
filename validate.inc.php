...
function ValidMail($email) {
	if (!preg_match("^[0-9a-z]([-_.]?[0-9a-z])*@([[:alnum:]]|xn--)([.-]?[[:alnum:]])*[.][a-wyz][[:alpha:]](f|g|l|m|t|u|v|z|ro|fo|op|pa|me|seum)?$^",(string) $email)){
		return FALSE;
	} else {
		return TRUE;
	}
...
function ValidDomain($domain) {
	if (!preg_match("^([[:alnum:]]|xn--)([.-]?[[:alnum:]])*[.][a-wyz][[:alpha:]](f|g|l|m|t|u|v|z|ro|fo|op|pa|me|seum)?$^",(string) $domain)){
		return FALSE;
	} else {
		return TRUE;
	}
...
function ValidAlias($domain,$alias) {
	$atpos=strpos((string) $alias,"@");
	$alias_domain=strpos((string) $alias,(string) $domain);
...
$reserved=explode(",",(string) $RESERVED);
#
$setforward = $forwardto = $_POST['setforward'] ?? '';
...
#OK############################# Check input if browse ################################################
	case "browse":
		if (!empty($_GET['orderby']) AND
			!in_array($_GET['orderby'], ['domain_name', 'prefix', 'maxaccounts', 'domainquota', 'quota'])){
			unset($_GET['orderby']);
		}
	...
	case "newdomain":
		if ($_SESSION['admintype'] != 0) {
			...
			if (!empty($_GET['confirmed'])){
				...
				} elseif (!in_array($_GET['transport'],["cyrus", "lmtp", "smtp", "uucp"])) {
					$authorized = FALSE;
...
#OK####################################### Check input if editalias ##################################
	case "editalias":
		...
		elseif (!empty($_GET['alias']) && (!empty($_GET['create']) && !ValidMail($_GET['alias'].'@'.$_GET['domain']) || empty($_GET['create']) && !ValidMail($_GET['alias']) && !ValidDomain(substr((string) $_GET['alias'],1)))) {
			$authorized = FALSE;
			$err_msg = _("Security violation detected, nothing deleted, attempt has been logged");
		} 
