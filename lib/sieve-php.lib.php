...
class sieve {
  ...
  function get_response()
  {
    ...
    if($this->token[0] == "NO"){
        ...
        $this->x = 0;
        [$this->ltoken, $this->mtoken, $this->rtoken] = explode(" ", $this->line." ", 3);
        if($this->mtoken[0] == "{"){
    ...
    elseif(str_starts_with($this->token[0], "OK")){
         return true;
    } /* end elseif */
    elseif($this->token[0][0] == "{"){
        ...
        $this->line = fgets($this->fp,1024);
        while(!str_starts_with($this->line, "OK") and !str_starts_with($this->line, "NO")){
            $this->response[]=$this->line;
            $this->line = fgets($this->fp, 1024);
        }
        if(str_starts_with($this->line, "OK"))
            return true;
        else
            return false;
    } /* end elseif */
    elseif($this->token[0][0] == "\""){
        ...
        while(!str_starts_with($this->line, "OK") and !str_starts_with($this->line, "NO")){
            ...
            [$this->ltoken, $this->rtoken] = explode(" ", $this->line." ",2);
    ...
    elseif(strstr($this->token[1], '(REFERRAL "' ) ){
        ...
        if(strstr((string) $this->capabilities['implementation'], 'v2.1')) {
    	...
    	if( sieve::sieve_login() ) {
    		fputs($this->fp, (string) $this->lastcmd);
   ...
   */
  function __construct(public $host, public $port, public $user, public $pass, $auth="", /**
   * a comma seperated list of allowed auth types, in order of preference
   */
  public $auth_types='PLAIN') {
    if(!strcmp((string) $auth, ""))		/* If there is no auth user, we deem the user itself to be the auth'd user */
        $this->auth = $this->user;
    else
        $this->auth = $auth;	/* Allowed authentication types */
    $this->fp=0;
    $this->line="";
    $this->retval="";
    $this->tmpfile="";
  ...
  function parse_for_quotes($string) {
      ...
      for($ptr = 0; $ptr < strlen((string) $string); $ptr++){
          if($string[$ptr] == '"' and $string[$ptr] != '\\'){
              if($start == -1){
                  $start = $ptr;
              } /* end if */
              else{
                  $token[$index++] = substr((string) $string, $start + 1, $ptr - $start - 1);
                  $found = true;
                  $start = -1;
              } /* end else */
  ...
  function status($string) {
      ...
      return match (substr((string) $string, 0,2)) {
          "NO" => F_NO,
          "OK" => F_OK,
          default => match ($string[0]) {
              "{" => F_HEAD,
              default => F_DATA,
          },
      };
  ...
  function sieve_login() {
    ...
    if(preg_match('#IMPLEMENTATION#m',$this->line))
    {
      ...
      while(sieve::status($this->line) == F_DATA){
          ...
          if(strcmp((string) $this->item[0], "IMPLEMENTATION") == 0)
              $this->capabilities["implementation"] = $this->item[1];

          elseif(strcmp((string) $this->item[0], "SIEVE") == 0 or strcmp((string) $this->item[0], "SASL") == 0){

              if(strcmp((string) $this->item[0], "SIEVE") == 0)
                  $this->cap_type="modules";
              else
                  $this->cap_type="auth";            

              $this->modules = explode(" ", (string) $this->item[1]);
          ...
          elseif(strcmp((string) $this->item[0], "STARTTLS") == 0) {
    ...
    else
    {
        ...
        $this->modules = substr((string) $this->item[1], strpos((string) $this->item[1], "{"),strlen((string) $this->item[1])-1);
    ...
    /* decision login to decide what type of authentication to use... */
     ...
     foreach(explode(" ", (string) $this->auth_types) as $auth_type)
     {
  ...
  function sieve_sendscript($scriptname, $script) {
    if($this->loggedin==false)
        return false;
    $this->script=stripslashes((string) $script);
  ...
  function authenticate() {
    ...
    switch ($this->auth_in_use) {
        ...
        case "DIGEST-MD5":
	     ...
	     $qop = explode(",",(string) $result['qop']);
	     if (!in_array($qop_value, $qop)) {
	        // rfc2831: client MUST fail if no qop methods supported
	        return false;
	     }
	     // build the $response_value
	     $string_a1 = mb_convert_encoding((string) $this->user, 'UTF-8', 'ISO-8859-1').":";
	     $string_a1 .= mb_convert_encoding((string) $result['realm'], 'UTF-8', 'ISO-8859-1').":";
	     $string_a1 .= mb_convert_encoding((string) $this->pass, 'UTF-8', 'ISO-8859-1');
	     $string_a1 = hmac_md5($string_a1);
	     $A1 = $string_a1.":".$result['nonce'].":".$cnonce.":".mb_convert_encoding((string) $this->auth, 'UTF-8', 'ISO-8859-1');
	     ...
	     $reply .= "qop=$qop_value,authzid=\"".mb_convert_encoding((string) $this->auth, 'UTF-8', 'ISO-8859-1')."\"";
	...
	case "LOGIN":
 	     $login=base64_encode((string) $this->user);
 	     $pass=base64_encode((string) $this->pass);
