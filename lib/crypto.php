...
class password{
	...
	*/
	...
	function update($table,$username,$newpassword,$encryption){
		...
		case "crypt":
		case "1":
                                $newsalt=preg_replace("/[^a-zA-Z0-9]+/", "", $newpassword);
                                $newpassword=crypt((string) $newpassword,substr((string) $newsalt,0,2));
		        break;
		case "md5":
                case "2":
			...
			case "1":
				...
				*/
				...
				#$password=crypt((string) $password,(string) $password);
	                        ...
	                        $newsalt=preg_replace("/[^a-zA-Z0-9]+/", "", $password);
        	                $password=crypt((string) $password,substr((string) $newsalt,0,2));
