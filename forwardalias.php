			...
			$daemon = new sieve($CYRUS['HOST'],"2000", $CYRUS['ADMIN'], $CYRUS['PASS'], $_GET['username']);
				...
				case 'set':
					if ($daemon->sieve_login()){
						...
						while (preg_match ("/(.*),(.*$)/U",(string) $forwardwhere, $matches)){
							$forwards_script .= 'redirect "' . trim($matches[1]) . '";'."\n";
							$forwardwhere = $matches[2];
						}
						$forwards_script .= 'redirect "' . trim((string) $forwardwhere) . '";';
						...
						if (preg_match ("/(require.*)(redirect \"|$)/Uis", (string) $old_script, $matches)){
