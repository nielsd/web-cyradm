...
if ($wc_configured){
	...
	if (isset($_SERVER['HTTP_ACCEPT_LANGUAGE'])){
		$browserlang = explode(',', (string) $_SERVER['HTTP_ACCEPT_LANGUAGE']);
	} else {
		$browserlang = 'en_EN';
	}
	...
	elseif (isset($nls['aliases'][substr($browserlang[0], 0, 2)])){
		...
		//$LANG = $DEFAULTLANG;
		$LANG = $nls['aliases'][substr((string) $DEFAULTLANG,0,2)];
	}
...
//		if (empty($_GET['domain']) && (empty($_GET['action']) || (! in_array($_GET['action'], array('logout', 'adminuser', 'newdomain', 'editadminuser', 'newadminuser', 'search'))))){
		...
		if (!isset($action) OR
		    empty($domain) AND !in_array($action, ['logout', 'adminuser', 'editadminuser', 'deleteadminuser', 'newadminuser', 'search', 'settings', 'changeadminpasswd', 'display', 'browse'])){
			...
			if (isset($_GET['action']) AND
			    in_array($_GET['action'], ['logout', 'browse', 'editdomain', 'manageaccount', 'newdomain', "deletedomain", "adminuser", "newadminuser", "editadminuser", "deleteadminuser", "editaccount", "newaccount", "deleteaccount", "vacation", "forwardalias", "forwardaccount", "newemail", "deleteemail", "editemail", "aliases", "newalias", "editalias", "deletealias", "search", "delete_catchall", "settings", "changeadminpasswd", "display"])){
			...
			# For password related stuff we also need to allow POST vars for some actions
				 ...
				 in_array($_POST['action'], ['newaccount', 'manageaccount', 'newadminuser', 'editadminuser', 'changeadminpasswd'])){
				include sprintf('%s/%s.php', WC_BASE, $_POST['action']);
			}
			else {
				match ($_GET['action']) {
        "accounts" => include WC_BASE . "/browseaccounts.php",
        "catch" => include WC_BASE . "/catchall.php",
        default => include WC_BASE . "/browse.php",
    };
			}
