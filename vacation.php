			...
			if (!empty($_GET['confirmed']) && empty($_GET['cancel'])){
				...
				case 'set':
					if ($daemon->sieve_login()){
						...
						$mess2 = preg_replace ("/\s*$/s",'',(string) $mess);
						$mess3 = preg_replace ("/\r/",'',(string) $mess2);
						if (preg_match ("/subject.*\n(.*)$/iUs", (string) $mess3, $matches)){
							...
							$subject = addslashes((string) $_GET['subject']);
							$text = addslashes((string) $mess2);
						}
						...
						if (preg_match("/redirect \".*$/siU", (string) $old_script, $matches)){
							$forwards_script = $matches[0];
						} else {
							$forwards_script ='';
						}
						$script = $vacation_script.$forwards_script;
						$script = iconv((string) $charset,"UTF-8",$script);
				...
				case 'unset':
					if ($daemon->sieve_login()){
						$sieve_str = new sieve_strs;
						$old_script = $sieve_str->get_old_script($daemon);
						if (preg_match ("/redirect \".*$/is",(string) $old_script,$matches)){
							$forwards_script = $matches[0];
				...
				if ($freeaddress!="YES") {
                            	    $aliasname = preg_split('#@#mi',(string) $_GET['alias'],2);
...
<?php
					if ($daemon->sieve_login()){
						$sieve_str = new sieve_strs;
						$old_script = iconv('UTF-8',(string) $charset,(string) $sieve_str->get_old_script($daemon));
					...
					if (count($matches) >= 2){
						$subject = stripslashes((string) preg_replace("/subject \"/", "", $matches[1]));
						$vacation_script = $matches[2];
						$vacation_script = stripslashes((string) preg_replace("/\";$/", "", $vacation_script));
					} else {
						$subject = '';
						$vacation_script = '';
					}
